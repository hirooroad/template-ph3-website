    ...
    <x-admin-sidebar />
      <main>
          <div class="container">
            ...
            <form class="question-form" method="POST" action="{{ route('admin_quiz_update', $quiz) }}" enctype="multipart/form-data">
            @csrf
            @method('PATCH')
              <div class="mb-4">
                <label for="question" class="form-label">問題文:</label>
                <input type="text" name="text" id="question" class="form-control required" value="{{$quiz->text}}" placeholder="問題文を入力してください" />
              </div>
